...
# This network is used to select actions during training and evaluation.
        # It learns to output the optimal action for a given state.
        # Interacts with the environment, generating actions for exploration or exploitation. 
        # Its parameters are updated by the gradient of the policy loss during training.
        
# This network provides stable action targets for the critic during training.
        # It is updated using a soft update to ensure consistency.
        # Provides target actions for the critic's loss calculation during training.
        # These target actions are computed as: a' = target_actor(s') where s' is the next state
        # The critic uses the target_actor to estimate the next action's value Q(s',a')
        # If this action is generated by a moving self.actor, the Q-value targets will have high variance.
