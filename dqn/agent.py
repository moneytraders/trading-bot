...
class DQNAgent:
    ...
    def setup(self, is_training, model_path):
        ...
        """
        df = load_data(f"../../../SOLUSDT/buckets/SOLUSDT-1m-{self.data_bucket}.csv")
        ...
        policy_dqn = DQN(self.window_size, num_observations, num_actions).to(device)

        if model_path is not None:
            policy_dqn.load_state_dict(torch.load(model_path, weights_only=True))

        if is_training:
            target_dqn = DQN(self.window_size, num_observations, num_actions).to(device)
            target_dqn.load_state_dict(policy_dqn.state_dict())
            optimizer = optim.Adam(
                policy_dqn.parameters(), lr=self.learning_rate_optimizer
            )
    ...
    def run(self, is_training, model_path):
        env, policy_dqn, target_dqn, optimizer = self.setup(is_training, model_path)
        ...
        for episode in itertools.count() if is_training else range(1):
            ...
            while not terminated and not truncated:
                ...
                if is_training and random.random() < epsilon:
                    ...
                    with torch.no_grad():
                        action = (
                            policy_dqn(state.unsqueeze(0).to(device)).squeeze().argmax()
                        )
                ...
                if is_training:
                    ...
                    if (
                        len(memory) >= self.minibatch_size
                        and step % self.optimize_every_steps == 0
                    ):
                        minibatch = memory.sample(self.minibatch_size)
                        self.optimize(optimizer, minibatch, policy_dqn, target_dqn)
            ...
            profit = percentage_increase(
                self.portfolio_initial_value, info["portfolio_valuation"]
            )
            if is_training:
                target_dqn.load_state_dict(policy_dqn.state_dict())
                ...
                if profit > best_profit:
                    best_profit = profit
                    torch.save(
                        policy_dqn.state_dict(), f"models/best_model_{profit}.pth"
                    )
                    ...
                    print(
                        f"Profit: {profit}, Validation profit: {validation_profit}, Epsilon: {epsilon}"
                    )

                epsilon_history.append(epsilon)
                rewards_per_episode.append(episode_reward)
            else:
                env.unwrapped.save_for_render(dir=self.render_dir)
                return profit
...
if __name__ == "__main__":
    ...
    agent = DQNAgent()
    c = agent.run(hyperparameters["is_training"], hyperparameters["model_path"])
    print(c)
